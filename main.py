...
url = "https://api.0x.org/orderbook/v1/orders"  # "https://api.0x.org/sra/v4/orders"
...
def test_arbitrage(maker_token, taker_token, estimated_fee, order):
    ...
    #     or order["feeRecipient"] != "0x0000000000000000000000000000000000000000"
    # ):
    #     return
    ...
    price = to_eth(taker_amount, taker_token) / to_eth(
        maker_amount, maker_token
    )  # price of bought token
    ...
    return_price = to_eth(expected_return, taker_token) / to_eth(
        maker_amount, maker_token
    )  # price of sold token

    profit = expected_return - taker_amount - estimated_fee
    print(f"Expected Profit for {to_eth_str(profit, taker_token)}")
...
def test_pair(maker_token, taker_token, estimated_fee):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor2:
        threads2 = [
            executor2.submit(
                test_arbitrage, maker_token, taker_token, estimated_fee, order
            )
            for order in orders
        ]
...
def execute():
    gas_price = requests.get(
        gas_station_url, params={"api-key": gas_station_api_key}
    ).json()["fastest"]
    print(f"Gas Price (fastest): {gas_price}")
    estimated_fee = estimated_gas * gas_price
    with concurrent.futures.ThreadPoolExecutor() as executor:
        threads = [
            executor.submit(test_pair, maker_token, taker_token, estimated_fee)
            for (maker_token, taker_token) in permutations(tokens, 2)
        ]
